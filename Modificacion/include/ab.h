...
class AB {
 public:
  ...
  //MODIFICACION
  int ContarPrimos() const {
    return ContarPrimosRama(raiz_);
  }
 ...
 protected:
  ...
  //MODIFICACION
  /**
    * @brief Recursively counts prime numbers in the tree
    */
  int ContarPrimosRama(NodoB<Key>* nodo) const {
    if (nodo == nullptr) return 0;

    int count = 0;

    if (EsPrimo(nodo->dato_.GetValue())) {
      count = 1;
    }

    return count + ContarPrimosRama(nodo->izdo_) + ContarPrimosRama(nodo->dcho_);
  }

  bool EsPrimo(long value) const {
